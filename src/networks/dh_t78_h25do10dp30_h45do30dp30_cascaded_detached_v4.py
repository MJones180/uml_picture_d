...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.mixer = nn.Sequential(
            nn.Linear(IN_DIM + OUTER_DIM, OUTER_DIM),
            nn.LeakyReLU(LEAKY_RELU),
            nn.Linear(OUTER_DIM, IN_DIM + OUTER_DIM),
            nn.LeakyReLU(LEAKY_RELU),
        )
    ...
    def forward(self, x):
        ...
        x_and_coupled = torch.cat([x, head_1_features.detach()], dim=1)
        mixed_features = self.mixer(x_and_coupled)
