# `dh_t68_h35dp10do30` network { 1000 -> 1000 }.
# Trainable parameters: 153,463,784
import numpy as np
import torch
...
DP_PROBS = np.linspace(0, DP_MAX_PROB, HEAD_DEPTH)
...
class Network(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.head_1_input = nn.Sequential(
            nn.Linear(IN_DIM, OUTER_DIM, bias=False),
            nn.BatchNorm1d(OUTER_DIM),
        )
        self.head_2_input = nn.Sequential(
            nn.Linear(IN_DIM, OUTER_DIM, bias=False),
            nn.BatchNorm1d(OUTER_DIM),
        )
    ...
    def forward(self, x):
        x_head_1 = self.head_1_blocks(self.head_1_input(x))
        x_head_2 = self.head_2_blocks(self.head_2_input(x))
