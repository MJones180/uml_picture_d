# `dh_t78_h25do10dp30_h45do30dp30_cascaded_detached_v3` network { 1000 -> 1000 }
# Trainable parameters: 59,131,344
import numpy as np
import torch
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.head_2 = nn.Sequential(
            nn.Linear(2 * IN_DIM, OUTER_DIM, bias=False),
            nn.BatchNorm1d(OUTER_DIM),
            *[
        ...
        self.feature_projector = nn.Linear(OUTER_DIM, IN_DIM)
    ...
    def forward(self, x):
        ...
        coupled_features = x * torch.tanh(
            self.feature_projector(head_1_features.detach()))
        x_and_coupled = torch.cat([x, coupled_features], dim=1)
        head_2_result = self.head_2_out(self.head_2(x_and_coupled))
