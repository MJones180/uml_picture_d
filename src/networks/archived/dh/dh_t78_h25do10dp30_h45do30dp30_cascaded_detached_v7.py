# `dh_t78_h25do10dp30_h45do30dp30_cascaded_detached_v7` network { 1000 -> 1000 }
# Trainable parameters: 59,487,928
import numpy as np
import torch
...
class MixerBlock(nn.Module):
    ...
    def forward(self, x):
        return self.gamma * self.block(x)
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        x_and_head1_features = torch.cat([x, head_1_features.detach()], dim=1)
        mixed_in = head_1_features.detach() + self.mixer(x_and_head1_features)
        head_2_input = torch.cat([x, mixed_in], dim=1)
        head_2_result = self.head_2_out(self.head_2(head_2_input))
