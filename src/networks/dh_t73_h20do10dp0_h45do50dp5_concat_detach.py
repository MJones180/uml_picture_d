# `dh_t73_h20do10dp0_h45do50dp5_concat_detach` network { 1000 -> 1000 }.
# Trainable parameters: 72,016,872
import numpy as np
import torch
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.head_2 = nn.Sequential(
            nn.Linear(IN_DIM + HEAD_OUT_DIM, OUTER_DIM, bias=False),
            nn.BatchNorm1d(OUTER_DIM),
            *[
    ...
    def forward(self, x):
        head_1_result = self.head_1_out(self.head_1(x))
        # Using detach prevents Head 2 loss from backprop into Head 1
        x_and_head_1_input = torch.cat([x, head_1_result.detach()], dim=1)
        head_2_result = self.head_2_out(self.head_2(x_and_head_1_input))
