...
def dm_comparison(cli_args):
    ...
    dm1_active_idxs = extra_vars_data[DM_ACTIVE_IDXS(0)][:]
    dm2_active_idxs = extra_vars_data[DM_ACTIVE_IDXS(1)][:]

    step_ri('Validating data')
    active_actuators = dm1_active_idxs.shape[0]
    if active_actuators != dm2_active_idxs.shape[0]:
        terminate_with_message('DMs must be the same size')
    elif 2 * active_actuators != outputs_model.shape[1]:
        terminate_with_message('Model outputs have wrong number of actuators')
    elif 2 * active_actuators != outputs_truth.shape[1]:
        terminate_with_message('Truth outputs have wrong number of actuators')
    ...
    def _prep_dm_cmd(values_1d):
        dm_cmd = np.zeros(dm_size**2)
        dm_cmd[dm1_active_idxs] = values_1d
        return dm_cmd.reshape(dm_size, dm_size)

    def _plot_row(outputs):
        return {
            'dm1': _prep_dm_cmd(outputs[:active_actuators]),
            'dm2': _prep_dm_cmd(outputs[active_actuators:]),
        }
