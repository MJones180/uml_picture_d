...
from loss_functions.landman_rms import LandmanRMS
...
def model_train(cli_args):
    ...
    if loss_name in ('mae', 'mse'):
        ...
        def loss_function(model_outputs, truth_outputs):
            ...
            return loss.mean()
    elif loss_name == 'landman_rms':
        print('Landman RMS')
        loss_obj = LandmanRMS(**loss_params)
    ...
    if opt_wd_one_d is not None or opt_wd_manual is not None:
        ...
        if opt_wd_one_d is not None:
            decay_params_one_d = []
            step('Will change weight decay for all 1D layers')
        ...
        if opt_wd_manual is not None:
            ...
            step('Will change weight decay for all specified layers')
